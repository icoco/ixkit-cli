...
import (
	...
	//"httplib"
	"github.com/icoco/ixkit-cli/vendors/astaxie/bat/httplib"
	//@
	//progress "github.com/raoulh/go-progress"
	progress "github.com/icoco/ixkit-cli/vendors/raoulh/go-progress"
)
...
func Main() (r *httplib.BeegoHttpRequest, d map[string][]interface{}) {
	//@
	...
	*URL = u.String()
	...
	httpreq, def := getHTTPWithDef(*method, *URL, args)
	if u.User != nil {
	...
	//@
	bodyMap := httpreq.Body2Map()
	if nil == bodyMap {
		bodyMap = make(map[string]interface{})
	}
	def["body"] = append(def["body"], bodyMap)
	...
	res, err := httpreq.Response() // print out request response
	if err != nil {
	...
	if download {
		//@@
		downloadAction(res, u.Path)
	}

	if runtime.GOOS != "windows" {
		printResponseOnLinux(httpreq, res)
	} else {
		printResponseOnWindows(httpreq, res)
	}

	//@ response
	body := formatResponseBody(res, httpreq, false)
	def["response"] = append(def["response"], body)

	def["url"] = append(def["url"], *URL)
	def["method"] = append(def["method"], *method)

	return httpreq, def
}

//@
func downloadAction(res *http.Response, path string) {
	var fl string
	...
	if fl == "" {
		_, fl = filepath.Split(path) //@ u.Path)
	}
...
func printResponseOnLinux(httpreq *httplib.BeegoHttpRequest, res *http.Response) {
...
func printResponseOnWindows(httpreq *httplib.BeegoHttpRequest, res *http.Response) {
...
func ValidateCURLArg(u Callback) bool {
	log.SetFlags(log.LstdFlags | log.Lshortfile | log.Lmicroseconds)
	flag.Usage = u //usage
	flag.Parse()
	args := flag.Args()
