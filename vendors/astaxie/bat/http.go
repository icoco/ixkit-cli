...
func getHTTPWithDef(method string, url string, args []string) (r *httplib.BeegoHttpRequest, d map[string][]interface{}) {
	...
	for i := range args {
		...
		if len(strs) >= 2 {
			...
			//def["headers"] = append(def["headers"], strings.Join([]string{strs[0], ":", strs[1]}, ""))
		...
		if len(strs) == 2 {
			...
			//def["parameters"] = append(def["parameters"], strings.Join([]string{strs[0], "=", strs[1]}, ""))
	...
	//@
	def["parameters"] = append(def["parameters"], parameters)
	def["headers"] = append(def["headers"], headers)
